  ...
  useEffect(() => {
    const fetchPendingLoans = async () => {
      const { data, error } = await supabase
        .from('LoanData')
        .select('*')
        .eq('status', 'Pending'); // Filter for pending loans
      ...
      setPendingLoans(data); // Update state with fetched data
    };
  ...
  return (
    <div className="min-h-screen flex pt-20">
      ...
      <main className="flex-1 p-6">
        <h1 className="text-2xl font-bold text-emerald-700">Pending Loan Requests</h1>
        <p className="mt-2 text-gray-600">List of pending loan applications will appear here.</p>

        {/* Table to display pending loan requests */}
          ...
          <table className="min-w-full text-left table-auto">
            ...
            <tbody>
              {pendingLoans.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-4 py-2 text-center text-gray-600">No pending loan requests found.</td>
                </tr>
              ) : (
